# -*- coding: utf-8 -*-
"""
dialogue_runtime 安装配置文件 (setup.py)
...
目录结构：
    ...
    ├── dialogue_runtime/                  <-- 要安装的 Python 包
    │   ├── __init__.py
    │   ├── agent/
    │   ├── core/
    │   └── ...
...
- dialogue_runtime/ 是我们要安装的包（里面有 __init__.py）
- 所以 setup.py 放在 dialogue_runtime/ 的父目录，即项目根目录
...
2. dialogue_runtime 包会被注册到 Python 环境，可以在任何地方 import
...
Q: 安装后怎么卸载？
A: pip uninstall dialogue_runtime

Q: 怎么查看是否安装成功？
A: pip show dialogue_runtime  或者  python -c "import dialogue_runtime; print(dialogue_runtime.__version__)"
"""
...
def read_version() -> str:
    """从 dialogue_runtime/__init__.py 读取版本号。
    
    为什么版本号定义在 __init__.py 而不是这里？
    - 这样代码里可以通过 dialogue_runtime.__version__ 获取版本
    - 版本号只需要维护一处，不会出现不一致
    """
    init_path = Path(__file__).parent / "dialogue_runtime" / "__init__.py"
...
setup(
    ...
    # 
    # include: 只包含这些包（dialogue_runtime 和它的所有子包）
    ...
    packages=find_packages(
        include=["dialogue_runtime", "dialogue_runtime.*"],
    ...
    # 命令行入口点（CLI）
    # ------------------------------------------------------------------------
    # 安装后会创建 atguigu 命令，执行时调用 dialogue_runtime.cli 模块的 main 函数
    # 
    # 格式："命令名=模块路径:函数名"
    # 安装后可以直接在终端运行：atguigu init / atguigu train / atguigu run
    entry_points={
        "console_scripts": [
            "atguigu=dialogue_runtime.cli:main",
        ],
    },
