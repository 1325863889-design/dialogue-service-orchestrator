...
```bash
# 训练模型
python -m dialogue_runtime train --config ecs_demo/config.yml

# 指定输出目录
python -m dialogue_runtime train --config ecs_demo/config.yml --out models/

# 带验证的训练
python -m dialogue_runtime train --config ecs_demo/config.yml --validate
```
...
| 方式 | 适用场景 | 命令 |
|------|----------|------|
| Shell | 开发调试 | `python -m dialogue_runtime shell ecs_demo/` |
| REST API | 生产环境 | `python -m dialogue_runtime run ecs_demo/` |
| Docker | 容器化部署 | `docker run -v ./ecs_demo:/app dialogue_runtime` |

### 17.2.2 启动服务

```bash
# 命令行交互模式
python -m dialogue_runtime shell ecs_demo/

# 启动REST API服务
python -m dialogue_runtime run ecs_demo/ --port 5005

# 从模型压缩包启动
python -m dialogue_runtime run models/model_20240115.tar.gz
```
...
CMD ["python", "-m", "dialogue_runtime", "run", "/app", "--port", "5005"]
```
