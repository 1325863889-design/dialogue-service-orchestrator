...
本文档涵盖 dialogue_runtime 对话系统的基础设施层，包括命令行工具、配置管理、异常处理、YAML工具和LLM客户端抽象等核心支撑模块。这些模块虽然不直接参与对话逻辑，但为整个系统提供了坚实的运行基础。
...
```python
# 文件: dialogue_runtime/cli/__init__.py
...
class AtguiguCLI(click.Group):
    ...
    def _load_commands(self) -> None:
        """加载所有可用的CLI命令。"""
        # 延迟导入避免循环依赖
        from dialogue_runtime.cli.init import init
        from dialogue_runtime.cli.train import train
        from dialogue_runtime.cli.run import run
        from dialogue_runtime.cli.shell import shell
        from dialogue_runtime.cli.inspect import inspect_cmd
        from dialogue_runtime.cli.export import export
...
```python
# 文件: dialogue_runtime/cli/init.py
...
from dialogue_runtime.core.actions import Action
from dialogue_runtime.core.events import SlotSet
from dialogue_runtime.shared.tracker import DialogueStateTracker
from dialogue_runtime.core.dispatcher import CollectingDispatcher
...
from dialogue_runtime.core.actions import Action
from dialogue_runtime.core.events import SlotSet, AllSlotsReset
from dialogue_runtime.shared.tracker import DialogueStateTracker
from dialogue_runtime.core.dispatcher import CollectingDispatcher
...
```python
# 文件: dialogue_runtime/cli/train.py
...
def train(
    ...
    # 执行训练
    try:
        from dialogue_runtime.training.trainer import train_model
...
```python
# 文件: dialogue_runtime/cli/run.py
...
def run(
    ...
    try:
        ...
        from dialogue_runtime.server.app import create_app
        import uvicorn
...
```python
# 文件: dialogue_runtime/cli/shell.py
...
from dialogue_runtime.shared.constants import DEFAULT_MODELS_PATH, DEFAULT_ENDPOINTS_PATH
...
```python
# 文件: dialogue_runtime/cli/inspect.py
...
本章详细介绍了dialogue_runtime的CLI命令行工具体系：
...
```python
# 文件: dialogue_runtime/shared/config.py
...
class TrackerStoreConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TrackerStoreConfig":
        ...
        return cls(
            ...
            database=data.get("database", "dialogue_runtime"),
...
```python
# 文件: dialogue_runtime/shared/constants.py
...
```python
# 文件: dialogue_runtime/shared/exceptions.py
...
```python
# 文件: dialogue_runtime/shared/yaml_loader.py
...
from dialogue_runtime.shared.constants import ENCODING_UTF8
...
```python
from dialogue_runtime.shared.yaml_loader import (
    read_yaml_file,
    write_yaml_file,
    merge_yaml_files,
)
...
```python
# 文件: dialogue_runtime/shared/llm/base_client.py
...
```python
from dialogue_runtime.shared.llm.base_client import (
    LLMClientFactory,
    Message,
)
...
```python
# 文件: dialogue_runtime/dialogue_understanding/commands/base.py
...
```python
# 文件: dialogue_runtime/channels/base_channel.py
...
```python
# 文件: dialogue_runtime/channels/rest_channel.py
...
```python
# 文件: dialogue_runtime/channels/socketio_channel.py
...
```python
# 文件: dialogue_runtime/cli/export.py
...
本文档详细介绍了dialogue_runtime对话系统的工具层与基础设施：
